...
\geometry{top=24mm,bottom=23mm,left=24mm,right=24mm,headheight=22pt}
\setmainfont[
...
\definecolor{notebg}{HTML}{F2F5FB}

\hypersetup{
  colorlinks=true,
  linkcolor=accent,
  urlcolor=accent,
  pdftitle={\detokenize{从 Infra 角度看，为什么去掉 Encoder 更好}}
}

\color{ink}
\setlength{\parindent}{0pt}
\setlength{\parskip}{0.68em}
\setlist[itemize]{leftmargin=1.7em,itemsep=0.3em,topsep=0.35em}

\titleformat{\section}
  {\Large\bfseries\sffamily\color{ink}}
  {}{0pt}{}
  [\vspace{0.25em}\color{rule}\titlerule\color{ink}]
\titlespacing*{\section}{0pt}{2.0em}{0.85em}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\footnotesize\sffamily\color{muted}Encoder-Free MLLM \textperiodcentered{} Purshow's Notes}
...
\newenvironment{CodeBlock}
  ...
  {\end{Verbatim}\endMakeFramed}

\newenvironment{PSNote}
  {\def\FrameCommand{\fboxsep=9pt\colorbox{notebg}}%
   \MakeFramed{\advance\hsize-2\width\FrameRestore}%
   \small\color{ink}\textbf{PS}\quad}
  {\endMakeFramed}

\begin{document}

{\fontsize{26}{34}\selectfont\sffamily\bfseries
从 Infra 角度看\\[0.08em]为什么去掉 Encoder 更好\par}

\vspace{0.9em}
{\color{accent}\rule{34mm}{2.2pt}}
\vspace{1.2em}
...
这种设计效果很好，LLaVA 几乎是引爆了多模态的热潮（最骄傲的信仰...)，但在大规模训练中，它带来的系统成本不只是多了一个几百 M 参数的 ViT。更重要的是，它在原本相对规则的 LLM 训练图中，插入了一段计算规模、输入形状和并行策略都不完全相同的计算路径。

\begin{PSNote}
本文主要讨论需要端到端 online 执行 Vision Encoder 的训练场景。如果 Encoder 完全冻结，且视觉特征可以离线预计算，下面的一些问题会明显减弱。
\end{PSNote}

\section{1. Vision Encoder 改变了原本规则的计算图}
\color{ink}
...
Projector 不是这篇文章的重点。在采用浅层 MLP Projector 的常见架构中，它的计算量通常远小于 Vision Encoder 和 LLM；如果使用 Q-Former 等更复杂的 Connector，则需要另行评估，不过目前应该也没有多少人再使用了。

\section{2. 多模态样本的计算成本更难预测}
\color{ink}
...
\begin{CodeBlock}
Cost(sample)
  approx. C_encoder(N_encoder_visual_tokens)
        + C_llm(N_text_tokens + N_llm_visual_tokens)
\end{CodeBlock}

其中，\texttt{N\_encoder\_}\allowbreak\texttt{visual\_tokens} 是进入 Vision Encoder 的 patch/frame token 数，\texttt{N\_llm\_}\allowbreak\texttt{visual\_tokens} 是经过 Projector 或 token merge 后送入 LLM 的视觉 token 数。

这会产生两类负载不均衡：

\begin{itemize}
  \item 不同 Data Parallel rank 拿到的图像分辨率、图片数量或视频长度不同，同步时需要等待最慢的 rank。
  \item 不同 micro-batch 的视觉负载不同，会让 Pipeline stage 的 service time 随时间波动。
\end{itemize}
...
\clearpage
\section{3. Vision Encoder 和 LLM 往往需要不同的并行策略}
\color{ink}
...
\texttt{T\_PP0} 不仅可能高于其他 stage，还会随 micro-batch 中的视觉负载变化。当 PP0 成为瓶颈时，下游 stage 会因为没有可消费的 micro-batch 而空转，并产生额外的 pipeline bubble 或 back-pressure。

再看 TP、CP 和模型放置。一个大型 MoE LLM 可能为了 hidden size、长上下文和 expert 参数量，使用较大的 TP、CP、PP 和 EP。但几百 M 参数的 Vision Encoder 往往不需要相同程度的切分：

\begin{itemize}
  \item 过大的 TP 可能让 Encoder 的单卡 GEMM 变小，通信占比反而上升。
  \item LLM 和 Vision Encoder 的序列长度与显存压力不同，最优 CP degree 往往也不同。
  \item 目前主流的 ViT 并不使用 MoE，因此 LLM 的 EP 并不适合 ViT。
\end{itemize}

训练系统因此面临一个取舍（当然，现在似乎已经有一些额外的处理方式，在此我就比较简化地来描述）：
...
\clearpage
\section{4. Encoder-free 解决了什么，又没有解决什么}
\color{ink}

Encoder-free 的系统价值，是拿掉了深层 ViT，让主要的视觉建模和文本建模复用同一套 decoder stack。从 Infra 角度看，这主要带来三个好处：

\begin{itemize}
  \item 减少 Encoder 和 LLM 之间的并行配置冲突。
  \item 减少只有部分样本才执行深层 Encoder 的条件分支。
  \item 让调度、profiling 和性能建模更接近现有的 LLM Infra。
\end{itemize}

当然，不同分辨率、图片数量和视频帧数仍然会让不同样本产生不同数量的视觉 token，因此样本之间的变长序列和负载差异不会完全消失。改善的是：这些 token 不再需要先经过一个深层、具有独立并行需求的 ViT。

\vfill
{\color{rule}\rule{\linewidth}{0.5pt}}\\[-0.1em]
{\footnotesize\sffamily\color{muted}Encoder-Free MLLM \textperiodcentered{} Purshow's Notes}
