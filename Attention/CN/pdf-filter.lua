-- Keep the Markdown title for normal renderers, but use the custom PDF cover.
local title_removed = false
local landscape_table_pending = false
local overview_table_done = false
local classification_table_done = false
...
function Header(header)
  ...
  if header_text:match("Attention 架构与参数总览") then
  ...
  -- The classification table may be commented out in the Markdown. In that
  -- case, close the landscape page before the next visible section.
  if overview_table_done and not classification_table_done
      and not header_text:match("Attention 类型分类") then
    classification_table_done = true
    return {
...
function Table(table)
  ...
  if overview_table_done and not classification_table_done then
    local widths = {0.18, 0.32, 0.50}
    for index, spec in ipairs(table.colspecs) do
      table.colspecs[index] = {spec[1], widths[index] or spec[2]}
    end
